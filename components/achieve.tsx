...
const events: EventData[] = [
  {
    ...
    description: "Our team pitching swahilipro for an opportunity to compete at the JKUAT Tech Expo 14.",  
    date: "September 2024",
    images: [
  ...
  },
  {
   ...
   title: "Jkuat Tech Expo 2024",
    description: "Our team presenting swahilipro at the Jkuat University Tech Expo 14 where we won first runners up.",
    date: "November 2024",
    images: [
  ...
  },
  {
    ...
    description: "At the Diplomacy Exhibition  interacting with various stakeholders,government officials and diplomats.",
    date: "December 2024",
    images: [
...
const AchievementsGallery: React.FC<AchievementsGalleryProps> = ({ className = '' }) => {
  ...
  const currentEvent = events.find(event => event.id === activeEvent) as EventData;
  ...
  return (
    <section className={`relative  pb-12 text-white overflow-hidden ${className}`}>
      {/* Decorative elements */}
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 opacity-20">
        <div className="absolute top-1/4 left-1/4 w-64 h-64 rounded-full bg-purple-600 blur-3xl"></div>
        <div className="absolute bottom-1/3 right-1/4 w-96 h-96 rounded-full bg-blue-600 blur-3xl"></div>
      </div>

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section header */}
        <div className="text-center mb-16">
          <h2 className="text-base font-semibold tracking-wider text-indigo-800 uppercase">Our Milestones</h2>
          <h3 className="mt-2 text-5xl font-extrabold text-black tracking-tight">Featured Events</h3>
        ...
        <div className="text-center mb-12">
          <h4 className="text-3xl font-bold text-black">{currentEvent.title}</h4>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {currentEvent.images.map((image, index) => (
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-tr from-indigo-900 to-purple-900 opacity-0 group-hover:opacity-30 transition-opacity duration-300 z-10"></div>
              <img
                src={image.src}
                alt={image.alt}
        ...
        {fullscreenImage && (
          ...
          >
            <div className="relative max-w-5xl max-h-screen">
              ...
              />
              <p className="text-white text-center mt-4">{fullscreenImage.alt}</p>
            </div>
          </div>
        )}
      </div>
